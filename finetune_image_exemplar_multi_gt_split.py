#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fine-tune SAMv3 exemplar detection/segmentation modules with data splits.

Dataset split behavior:
    This is the split-aware copy of ``finetune_image_exemplar_multi_gt.py``.
    It can train on all entries from ``--dataset_root`` as before, or use
    explicit frame-level train/validation/test CSV files. The split path is
    enabled by either:

    * passing ``--split_dir DIR`` so the script creates or reuses
      ``DIR/train.csv``, ``DIR/val.csv``, and ``DIR/test.csv``; or
    * passing one or more explicit CSV paths through ``--train_split_csv``,
      ``--val_split_csv``, and ``--test_split_csv``.

    Split CSVs are intentionally keyed by ``frame_id`` rather than by object
    instance. This keeps the same synthetic scene/frame in the same split
    across every camera folder in ``--dataset_root``. For multi-camera data,
    that avoids leakage where one view of a scene is used for training while
    another view of the same scene is used for validation or test.

    When ``--split_dir`` is provided and all three CSV files already exist, the
    existing split is reused for reproducibility. If only some files exist, the
...
def main() -> None:
    ...
    train_frames = sorted({entry["frame_id"] for entry in all_entries})
    print(f"Training entries: {len(all_entries)} frames/views from {len(train_frames)} unique frame ids.")
