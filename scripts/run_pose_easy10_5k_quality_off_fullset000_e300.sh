...
MANIFEST="${MANIFEST:-${REPO_ROOT}/runs/manifests/pose_easy10_5k_v2/manifest.csv}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/runs/cad_pose_easy10_5k_quality_off_fullset000_e300}"
DEVICE="${DEVICE:-cuda:0}"
EPOCHS="${EPOCHS:-300}"
PYTHON_BIN="${PYTHON_BIN:-${REPO_ROOT}/.venv/bin/python}"
GPU_MEMORY_START_THRESHOLD_MIB="${GPU_MEMORY_START_THRESHOLD_MIB:-8000}"
GPU_POLL_SECONDS="${GPU_POLL_SECONDS:-60}"

while true; do
  GPU_MEMORY_USED_MIB="$(nvidia-smi --id=0 --query-gpu=memory.used --format=csv,noheader,nounits | tr -d '[:space:]')"
  if [[ "${GPU_MEMORY_USED_MIB}" =~ ^[0-9]+$ ]] && (( GPU_MEMORY_USED_MIB <= GPU_MEMORY_START_THRESHOLD_MIB )); then
    break
  fi
  echo "Waiting for GPU 0 memory: used=${GPU_MEMORY_USED_MIB} MiB threshold=${GPU_MEMORY_START_THRESHOLD_MIB} MiB"
  sleep "${GPU_POLL_SECONDS}"
done

# Clean loss ablation: retain the original decomposed geometry objective while
# disabling both the pose-quality and coupled full-placement objectives.
